...
//export nominee
exports.nomineeExport = async (req, res) => {
    try {
        const nomineedata = await Nominee.find();
        ...
        const writablestream = fs.createWriteStream(
            "public/files/export/nominees.csv"
        )
        csvStream.pipe(writablestream);

        writablestream.on("finish", function () {
            res.json({
                downloadUrl: `${BASE_URL}/files/export/nominees.csv`
            })
        });
        ...
        if (nomineedata.length > 0) {
            nomineedata.map((nominee) => {
                csvStream.write({
                    FirstName: nominee.fname ? nominee.fname : "-",
                    LastName: nominee.lname ? nominee.lname : "-",
                    BirthDate: nominee.bdate ? moment(nominee.bdate).format("DD-MMM-YYYY"): "-",
                    Department: nominee.dept ? nominee.dept : "-",
                    Email: nominee.email ? nominee.email : "-",
                    Mobile: nominee.mobile ? nominee.mobile : "-",
                    PRN: nominee.prn ? nominee.prn : "-",
                    Post: nominee.post ? nominee.post : "-",
                    Vote_Count: nominee.voteCount ? nominee.voteCount : "-",
...
async function getNomineeInfoById(nomineeId)
{
    try{
...
exports.voteExport = async (req, res) => {
    ...
    try {
        ...
        const user = await users.findById(userId);
         for(const votedNominee of user.votedNominees)
         {
            if(votedNominee.post === post)
            {
                const alreadyVotednominee = await getNomineeInfoById(votedNominee.nomineeId);
                const nomineeName = `${alreadyVotednominee.fname}  ${alreadyVotednominee.lname}`;
                return res.status(405).json({message: `You have already voted to ${nomineeName} in this post`, NomineeInformation: nomineeName});
            }
         }
        ...
        const nomineeInfo = {
            nomineeId: nominee._id, // Replace 'nominee' with the voted nominee's document
            post: nominee.post,
            fname: nominee.fname,
            lname: nominee.lname,
            // Add any other nominee information you want to store
        };
        
        user.votedNominees.push(nomineeInfo);
        await user.save();
...
exports.userExport = async (req, res) => {
    try {
        ...
        if (usersdata.length > 0) {
            usersdata.map((user) => {
                const votedNomineesFnames = user.votedNominees.map((nominee) => nominee.fname + " " +  nominee.lname).join(', ');
                csvStream.write({
                    FirstName: user.fname ? user.fname : "-",
                    LastName: user.lname ? user.lname : "-",
                    BirthDate: user.bdate ? moment(user.bdate).format("DD-MMM-YYYY") : "-",
                    Department: user.dept ? user.dept : "-",
                    Email: user.email ? user.email : "-",
                    Mobile: user.mobile ? user.mobile : "-",
                    PRN: user.prn ? user.prn : "-",
                    Voting_Status: user.votedNominees.length > 0 ? "Voted To" : "Not Voted",
                    Voted_Nominees: user.votedNominees.length > 0 ? votedNomineesFnames : "-",
...
//export winner
exports.winnerExport = async (req, res) => {
    try {
       
        const winnerdata = winnersnominees;
        ...
        const writablestream = fs.createWriteStream(
            "public/files/export/winners.csv"
        )
        csvStream.pipe(writablestream);

        writablestream.on("finish", function () {
            res.json({
                downloadUrl: `${BASE_URL}/files/export/winners.csv`
            })
        });

        if (winnerdata.length > 0) {
            winnerdata.map((nominee) => {
                csvStream.write({
                    FirstName: nominee.winner.fname ? nominee.winner.fname : "-",
                    LastName: nominee.winner.lname ? nominee.winner.lname : "-",
                    BirthDate: nominee.winner.bdate ? moment(nominee.winner.bdate).format("DD-MMM-YYYY"): "-",
                    Department: nominee.winner.dept ? nominee.winner.dept : "-",
                    Email: nominee.winner.email ? nominee.winner.email : "-",
                    Mobile: nominee.winner.mobile ? nominee.winner.mobile : "-",
                    PRN: nominee.winner.prn ? nominee.winner.prn : "-",
                    Post: nominee.winner.post ? nominee.winner.post : "-",
                    Vote_Count: nominee.winner.voteCount ? nominee.winner.voteCount : "-",
